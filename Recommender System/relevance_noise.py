...
def add_noise(df, flip_p=0.3, cont_frac=0.3, thematic_flip_p=0.1, rng=None):
    ...
    # Missing data flips (if these columns exist)
    if "hasName" in df.columns:
        mask = rng.random(len(df)) < flip_p
        df.loc[mask, "hasName"] = 1 - df.loc[mask, "hasName"]
    if "hasOpeningHours" in df.columns:
        mask = rng.random(len(df)) < flip_p
        df.loc[mask, "hasOpeningHours"] = 1 - df.loc[mask, "hasOpeningHours"]
